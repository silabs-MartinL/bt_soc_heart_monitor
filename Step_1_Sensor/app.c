...
static uint8_t log_streams = (LOG_SENSOR);
  ...
  else
  {
    ...
    if (SL_STATUS_OK == sensor_status)
    {
      ...
      if (sensor_data.status == 3 && sensor_data.confidence > 0 && sensor_data.heart_rate > 0)
      {
        ...
        if (sensor_data.heart_rate < 30) sensor_pulse_ms = 60000 / 30;
        else if (sensor_data.heart_rate > 240) sensor_pulse_ms = 60000 / 240;
        else sensor_pulse_ms = 60000 / sensor_data.heart_rate;
      }
