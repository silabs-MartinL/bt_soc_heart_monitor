...
static sl_status_t app_sensor_configure(void)
{
  ...
  if (log_streams & LOG_SENSOR)
    app_log("    bio_hub_set_pulse_width(%d)=0x%04x\r\n", sensor_pulse_widths[sensor_pulse_width_index],
        (uint16_t )status);
  if (SL_STATUS_OK == status)
  {
    ...
    if (log_streams & LOG_SENSOR)
    app_log("    bio_hub_read_pulse_width()=0x%04x pulse_width=%u\r\n", (uint16_t )status, pulse_width);
    if (SL_STATUS_OK == status)
    {
      ...
      if (log_streams & LOG_SENSOR)
        app_log("    bio_hub_set_sample_rate(%d)=0x%04x\r\n", sensor_sample_rates[sensor_sample_rate_index],
            (uint16_t )status);
      if (SL_STATUS_OK == status)
      {
        ...
        if (log_streams & LOG_SENSOR)
        app_log("    bio_hub_read_sample_rate()=0x%04x sample_rate=%u\r\n", (uint16_t )status, sample_rate);
        // Clear flag
  ...
  case sl_bt_evt_gatt_server_characteristic_status_id:
    ...
    if (gattdb_heart_rate_measurement == evt->data.evt_gatt_server_characteristic_status.characteristic)
    {
      if (log_streams & LOG_BLUETOOTH)
        app_log("sl_bt_on_event(gatt_server_characteristic_status, heart_rate_measurement)");
...
static sl_status_t app_update_heart_rate_measurement_characteristic(void)
{
  ...
  if (bt_system_boot)
  {
    ...
    if (3 == sensor_data.status)
        data_send[0] |= 0x04; // SHOULD BE 0x02 FROM SPEC
    ...
    sc = sl_bt_gatt_server_write_attribute_value(gattdb_heart_rate_measurement, 0, sizeof(data_send), data_send);
    if (log_streams & LOG_BLUETOOTH)
      app_log("    sl_bt_gatt_server_write_attribute_value(0x%02x %d)=0x%04x\r\n", data_send[0], data_send[1],
          (uint16_t )sc);
  }
...
static sl_status_t app_send_heart_rate_measurement_notification(void)
{
  ...
  if (bt_system_boot)
  {
    ...
    sc = sl_bt_gatt_server_read_attribute_value(gattdb_heart_rate_measurement, 0, sizeof(data_send), &data_len,
        data_send);
...
static void app_pulse_timer(sl_simple_timer_t *timer, void *data)
{
  ...
  // Start pulse timer
  sl_simple_timer_start(&pulse_timer, sensor_pulse_ms, app_pulse_timer, NULL, false);
  // Update GATT data
